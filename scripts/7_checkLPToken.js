...
let SwappiPair = require(`../test/SwappiPair.sol/SwappiPair.json`);
...
async function main() {
  ...
  let LPAddr = await swappiFactoryContract.getPair(newTokenAddr, addresses.WCFX, {gasLimit: 1000000,});
  console.log("LP Address:", LPAddr);
  let SwappiPairContract = new ethers.Contract(LPAddr, SwappiPair.abi, admin);
  let LPTokenAmt = await SwappiPairContract.balanceOf(tokenOwner.address, {gasLimit: 1000000,});
